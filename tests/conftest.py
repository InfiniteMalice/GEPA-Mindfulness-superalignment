...
def _optional_import(module_name: str) -> Any:
    """Attempt to import optional dependency without raising."""

    import importlib

    spec = importlib.util.find_spec(module_name)
    if spec is None:  # pragma: no cover - dependency truly absent
        return None
    try:
        return importlib.import_module(module_name)
    except ImportError:  # pragma: no cover - partial install issues
        return None


torch = _optional_import("torch")
transformers = _optional_import("transformers")
mindful_trace_gepa = _optional_import("mindful_trace_gepa")
