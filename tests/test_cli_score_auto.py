...
from mindful_trace_gepa.scoring import DEFAULT_CONFIG as SCORING_DEFAULT_CONFIG
...
def test_build_config_sanitizes_numeric_values():
    ...
    assert config["weights"]["classifier"] == SCORING_DEFAULT_CONFIG["weights"]["classifier"]
    assert config["abstention_thresholds"]["mindfulness"] == 0.8
    expected_integrity = SCORING_DEFAULT_CONFIG["abstention_thresholds"]["integrity"]
    assert config["abstention_thresholds"]["integrity"] == expected_integrity
    assert config["disagreement_penalty"] == 0.5
    assert config["escalate_if_any_below"] == SCORING_DEFAULT_CONFIG["escalate_if_any_below"]
