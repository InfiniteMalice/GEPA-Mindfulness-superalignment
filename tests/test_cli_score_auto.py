...
import mindful_trace_gepa.scoring as scoring
...
def test_build_config_sanitizes_numeric_values():
    ...
    assert config["weights"]["classifier"] == scoring.DEFAULT_CONFIG["weights"]["classifier"]
    assert config["abstention_thresholds"]["mindfulness"] == 0.8
    expected_integrity = scoring.DEFAULT_CONFIG["abstention_thresholds"]["integrity"]
    assert config["abstention_thresholds"]["integrity"] == expected_integrity
    assert config["disagreement_penalty"] == 0.5
    assert config["escalate_if_any_below"] == scoring.DEFAULT_CONFIG["escalate_if_any_below"]
