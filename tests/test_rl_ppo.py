...
def test_ppo_clipped_surrogate_matches_manual_values() -> None:
    ...
    evaluation = PolicyEvaluation(log_probs=torch.log(torch.tensor([1.5, 0.5])))
...
def test_ppo_entropy_and_kl_contribute_with_configured_signs() -> None:
    batch = _batch(advantages=torch.zeros(2), returns=None, old_values=None)
    evaluation = PolicyEvaluation(
    ...
    # KL estimator values are 0 and exp(1)-2; entropy mean is 3.
    expected_kl = (torch.exp(torch.tensor(1.0)).item() - 2.0) / 2.0
...
def test_ppo_rejects_mask_that_selects_no_tokens() -> None:
    ...
    evaluation = PolicyEvaluation(log_probs=torch.zeros(2))
...
def test_ppo_rejects_evaluation_shape_mismatch() -> None:
    ...
    with pytest.raises(ValueError, match="matching shapes"):
        compute_ppo_loss(
            OPS,
            batch,
            PolicyEvaluation(log_probs=torch.zeros(3)),
            PPOAlgorithmConfig(),
        )
...
def test_ppo_loss_is_finite_and_preserves_dtype_and_autograd() -> None:
    ...
    evaluation = PolicyEvaluation(log_probs=log_probs)
...
def test_ppo_algorithm_adapts_canonical_runtime_config() -> None:
    ...
    batch = _batch(advantages=torch.tensor([1.0, 1.0]), returns=None, old_values=None)
    log_probs = torch.log(torch.tensor([2.0, 2.0]))
    evaluation = PolicyEvaluation(log_probs=log_probs, reference_log_probs=log_probs)
