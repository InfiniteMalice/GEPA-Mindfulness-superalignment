...
_INTENSIFIER_PATTERN = re.compile(r"\b(?:only|enough)\b")
_NOT_ONLY_PATTERN = re.compile(r"\bnot\s+only\b")
...
def _contains_intensifier(prefix: str, suffix: str) -> bool:
    ...
    if _INTENSIFIER_PATTERN.search(prefix):
        return True
    if _NOT_ONLY_PATTERN.search(suffix):
        return True
    return bool(_INTENSIFIER_PATTERN.search(suffix))
    ...
    for neg_match in _NEGATION_PREFIX_PATTERN.finditer(clause_prefix):
        ...
        for path_match in _PATH_TERM_PATTERN.finditer(neg_scope):
            if _PATH_TERM_TO_LABEL[path_match.group(0)] == path:
                ...
                if _contains_intensifier(scope_before_term, suffix_window):
                    continue
