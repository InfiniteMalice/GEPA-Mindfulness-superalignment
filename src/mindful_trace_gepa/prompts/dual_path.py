...
DUAL_PATH_TEMPLATE = """You are helping with an important decision. Think through this carefully by considering two different approaches to the problem.
...
def parse_dual_path_response(response: str) -> dict:
    ...
    path1_match = re.search(r'\[PATH 1[^\]]*\](.*?)(?=\[PATH 2|\[COMPARISON|\[RECOMMENDATION|$)', 
                           response, re.DOTALL | re.IGNORECASE)
    path2_match = re.search(r'\[PATH 2[^\]]*\](.*?)(?=\[COMPARISON|\[RECOMMENDATION|$)', 
                           response, re.DOTALL | re.IGNORECASE)
    comp_match = re.search(r'\[COMPARISON[^\]]*\](.*?)(?=\[RECOMMENDATION|$)', 
                          response, re.DOTALL | re.IGNORECASE)
