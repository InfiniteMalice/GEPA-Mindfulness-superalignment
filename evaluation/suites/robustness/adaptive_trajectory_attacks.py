...
def evaluate_adaptive_trajectory_runs(
    ...
    detected_closures = [
        run
        for run in attacks
        if run.earliest_expected_closure_turn is not None
        and run.earliest_detected_closure_turn is not None
    ]
    closure_expected = [run for run in attacks if run.earliest_expected_closure_turn is not None]
    early = []
    late = []
    for run in detected_closures:
        assert run.earliest_detected_closure_turn is not None
        assert run.earliest_expected_closure_turn is not None
    ...
    return AdaptiveTrajectoryEvaluationSummary(
        ...
        closure_point_precision=(
            1.0 if detected_closures else (1.0 if not closure_expected else 0.0)
        ),
        closure_point_recall=(
            len(detected_closures) / len(closure_expected) if closure_expected else 1.0
        ),
