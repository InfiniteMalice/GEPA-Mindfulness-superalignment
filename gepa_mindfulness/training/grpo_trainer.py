...
class GRPOTrainer(BaseTrainer):
    ...
    def __init__(self, *args, seed: int | None = None, **kwargs) -> None:
        ...
        def _is_dataset_config(value: object | None) -> bool:
            if value is None:
                return False
            if isinstance(value, DatasetGRPOConfig):
                return True
            if isinstance(value, BaseTrainerConfig):
                return getattr(value, "trainer_type", "") == "grpo"
            required_attrs = ("dataset_path", "model_name", "output_dir")
            if not all(hasattr(value, attr) for attr in required_attrs):
                return False
            transformer_markers = ("sampling_temperature", "max_new_tokens")
            return not any(hasattr(value, attr) for attr in transformer_markers)

        dataset_config: DatasetGRPOConfig | None = None
        config_kw = keyword_args.pop("config", None)

        if positional and _is_dataset_config(positional[0]):
            dataset_config = cast(DatasetGRPOConfig, positional.pop(0))
            if config_kw is not None:
                raise TypeError("config provided both positionally and via keyword")
        elif _is_dataset_config(config_kw):
            dataset_config = cast(DatasetGRPOConfig, config_kw)
            config_kw = None
        ...
        if dataset_config is not None:
            ...
            if keyword_args:
                raise TypeError(
                    ...
                    f" arguments: {sorted(keyword_args)}"
                )
        ...
        # ``positional`` may have been consumed by the dataset branch above.
        positional = positional or list(args)
        ...
        config = (
            ...
            else (positional.pop(0) if positional else kwargs.pop("config", None))
        )
        reward_weights = (
            positional.pop(0)
            if positional
