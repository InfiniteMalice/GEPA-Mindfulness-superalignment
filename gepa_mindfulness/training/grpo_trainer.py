...
class GRPOTrainer(BaseTrainer):
    ...
    def __init__(self, *args, seed: int | None = None, **kwargs) -> None:
        ...
        def _as_dataset_config(value: object | None) -> DatasetGRPOConfig | None:
            if value is None:
                return None
            if isinstance(value, DatasetGRPOConfig):
                return value
            if isinstance(value, BaseTrainerConfig) and getattr(
                value, "trainer_type", ""
            ) == "grpo":
                return cast(DatasetGRPOConfig, value)
            return None

        dataset_config: DatasetGRPOConfig | None = None

        if positional:
            candidate = _as_dataset_config(positional[0])
            if candidate is not None:
                ...
                if "config" in kwargs:
                    raise TypeError(
                        "config provided both positionally and via keyword"
                    )

        if dataset_config is None and "config" in kwargs:
            candidate = _as_dataset_config(kwargs.get("config"))
            if candidate is not None:
                if positional:
                    raise TypeError(
                        "Dataset-driven GRPOTrainer does not accept positional model"
                        " arguments"
                    )
                dataset_config = candidate
                kwargs.pop("config")
        ...
        transformers_config = kwargs.pop("config", None)
