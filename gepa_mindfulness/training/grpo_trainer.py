...
class GRPOTrainer(BaseTrainer):
    ...
    def __init__(self, *args, seed: int | None = None, **kwargs) -> None:
        ...
        if args and isinstance(args[0], DatasetGRPOConfig):
            ...
            candidate = kwargs.pop("config")
            if isinstance(candidate, DatasetGRPOConfig) and not isinstance(
                candidate, TransformersGRPOConfig
            ):
                if args:
                    raise TypeError(
                ...
                # Put the value back for the HF code path below.
                kwargs["config"] = candidate
